...
void preorder(struct rec *tree); void inorder(struct rec "tree); void postorder(struct rec *tree);
int count=1;
main()
{
    int choice;
...
while((selection<1)]](selection>5));
            ...
            {
                if (tree == NULL)
                    1 tree = (struct rec *)malloc(sizeof(struct rec));
        ...
        void preorder(struct rec * tree)
        {
            ...
            void inorder(struct rec * tree) 1 if (tree != NULL)
            {
                inorder(tree->left);
                printf("%12ld\n", tree->num);
                inorder(tree->right);
